    ...
    def __new__(cls):
        if not hasattr(cls, "instance"):
            ...
            cls.instance.mongo_client = motor_asyncio.AsyncIOMotorClient(
                MONGO_URI, driver = DRIVER_INFO
            )
            cls.instance.engine = AIOEngine(client=cls.instance.mongo_client, database=DB_NAME)
        return cls.instance 


def MongoDatabase() -> core.AgnosticDatabase:
    return _MongoClientSingleton().mongo_client[DB_NAME]
